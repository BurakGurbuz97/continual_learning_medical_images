...
# ClassifierF
class Vgg11_AfterLayerX(nn.Module):
    def __init__(self, backbone):
        super(Vgg11_AfterLayerX, self).__init__()
        self.model = copy.deepcopy(backbone)

        for x in range(0,16 ): # till 4th MaxPool layer
            del self.model.conv[0]
        
    def forward(self, x):
        out = self.model.conv[0](x)
        for module in self.model.conv:
            x = module(x)
        # Flatten the output of the convolutional layers
        x = x.flatten(start_dim=1)

        for module in self.model.hidden_layers:
            x = module(x)
...
# ClassifierG
class Vgg11_tilllayerX(nn.Module):
    def __init__(self, backbone , num_del=5):
        super(Vgg11_tilllayerX, self).__init__()
        self.model = copy.deepcopy(backbone.conv)
        for _ in range(0, num_del):
            del self.model[-1]
        # del self.model.hidden_layers
...
# class Vgg11ClassifyAfterLayer16(BaseVgg11ClassifyAfterLayerX):
#     def __init__(self, backbone ,  num_classes=None):
#         super(BaseVgg11ClassifyAfterLayerX, self).__init__(backbone , num_del=16)

# class Vgg11ClassifyAfterLayer10(BaseVgg11ClassifyAfterLayerX):
#     def __init__(self, backbone ,  num_classes=None):
#         super(BaseVgg11ClassifyAfterLayerX, self).__init__(backbone)
...
class Remind(NaiveContinualLearner):
    ...
    def fit_both_models_jointly(self, train_loader, val_loader, max_pretrain_epoch):
        ...
        # make the optimizer
        optimizer = optim.Adam(trainable_params, weight_decay=1e-5)
        ...
        self.classifier_F = Vgg11_AfterLayerX(backbone = model).to(self.device)
